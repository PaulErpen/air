...
class ModelKNRMTest(unittest.TestCase):
    def setUp(self):
        ...
        _triple_reader = IrLabeledTupleDatasetReader(lazy=True, max_doc_length=180, max_query_length=30)
        _triple_reader = _triple_reader.read("./test_data/validation_test.tsv")
        _triple_reader.index_with(self.vocab)
        self.validation_loader = PyTorchDataLoader(_triple_reader, batch_size=32)
    ...
    def test_train_batch(self):
        self.assertCountEqual(self.train_batch.keys(), ["query_tokens", "doc_pos_tokens", "doc_neg_tokens"])

    def test_validation_batch(self):
        self.assertCountEqual(self.validation_batch.keys(), ["query_id", "doc_id", "query_tokens", "doc_tokens"])
    ...
    def test_embedding(self):
        ...
        self.assertEqual(model.word_embeddings(self.train_batch["query_tokens"]).shape, (32, 14, 300))
    
    def test_translation_matrix(self):
        model = KNRM(word_embeddings=self.word_embedder, n_kernels=10) # type: ignore

        self.assertEqual(
            model.create_translation_matrix(self.train_batch["query_tokens"], self.train_batch["doc_pos_tokens"]).shape, # type: ignore
            (32, 14, 123)
        )

    def test_gaussian(self):
        model = KNRM(word_embeddings=self.word_embedder, n_kernels=10) # type: ignore
        translation_matrix = model.create_translation_matrix(self.train_batch["query_tokens"], self.train_batch["doc_pos_tokens"]) # type: ignore
    ...
    def test_apply_kernel_functions(self):
        model = KNRM(word_embeddings=self.word_embedder, n_kernels=10) # type: ignore
        translation_matrix = model.create_translation_matrix(self.train_batch["query_tokens"], self.train_batch["doc_pos_tokens"])
